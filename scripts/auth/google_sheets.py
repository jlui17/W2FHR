...
STAFF_LIST_SHEET_ID = "1--oCY9YLsg7dctcQaLXayx98sx6ZeuLh-GgQHuUH3ck"
...
SCHEDULE_SHEET_ID = "1cU3W4DrRVk17hdDWKR5cbppFe1sekHvLX1pXVC6gTCs"
...
if os.path.exists(GOOGLE_SHEETS_SECRET_FILE_NAME):
    ...
    try:
        print(
            "No Google Service account credentials found. Attempting to retrieve them..."
        )
        ...
        f.close()
        print(
            "Successfully retrieved Google Service account credentials and saved them."
        )
        cred = service_account.Credentials.from_service_account_info(
            json.loads(G_SERVICE_CONFIG)
        )
        client = build("sheets", "v4", credentials=cred).spreadsheets()
...
def get_employee_ids():
    result = (
        client.values()
        .get(
            spreadsheetId=STAFF_LIST_SHEET_ID,
            range=STAFF_LIST_EMPLOYEE_ID_RANGE,
...
def get_positions():
    result = (
        ...
        .get(
            spreadsheetId=STAFF_LIST_SHEET_ID,
            range=STAFF_LIST_POSITION_RANGE,
            majorDimension="COLUMNS",
        )
        .execute()
    )
...
def get_availability_employee_ids():
    result = (
        ...
        .get(
            ...
            range=SCHEDULE_AVAILABILITY_EMPLOYEE_ID_RANGE,
            majorDimension="COLUMNS",
        )
        .execute()
    )
