...
import { ManagerTimesheetData } from "../Timesheet/ManagerTimesheet/ManagerTimesheetController";

interface UpcomingShiftsWidgetProps {
  isLoading: boolean;
  upcomingShiftsData: TimesheetData | ManagerTimesheetData;
}
...
export const UpcomingShiftsWidget = ({
  ...
  const upcomingShifts = () => {
    if (upcomingShiftsData as TimesheetData) {
      const data = upcomingShiftsData as TimesheetData;

      return (
        <div className="mt-2 flex flex-col">
          {data.shifts.map((shift: Shift, i: number) => {
            return (
